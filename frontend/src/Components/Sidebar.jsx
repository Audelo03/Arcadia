...
import { FaBars, FaRoute, FaSignOutAlt, FaTrashAlt } from "react-icons/fa";
...
const Sidebar = ({ onTogglePredefinedRoutes, arePredefinedRoutesVisible }) => {
  ...
  const [connectionAttemptTimeoutId, setConnectionAttemptTimeoutId] =
    useState(null); // Renombrado para claridad
        ...
        try {
          ...
          if (message.type === "gps_status") {
            ...
            if (status === "waiting_for_valid_data") {
              ...
              setGpsMessageText(
                statusMessage || "Esperando datos GPS válidos..."
              );
          ...
          console.error(
            "Error processing WebSocket message in Sidebar:",
            error
          );
        }
      };
  ...
  const disconnectGps = useCallback(() => {
    ...
    fetch("http://localhost:3001/api/disconnect-gps")
      ...
      .then((data) => {
        setGpsMessageText(
          data.message || "Solicitud de desconexión GPS enviada."
        );
        setShowGpsMessage(true);
      })
      .catch((error) => {
  ...
  const handleExternalGpsClick = useCallback(() => {
    ...
    setIsAttemptingConnection(true);
    setGpsConnected(false);
    setGpsMessageText("Iniciando conexión GPS...");
    setShowGpsMessage(true);

    const timeoutId = setTimeout(() => {
      if (isAttemptingConnection && !gpsConnected) {
        setGpsMessageText(
          "Error: Tiempo de espera agotado. Verifica el dispositivo GPS."
        );
        setShowGpsMessage(true);
        setIsAttemptingConnection(false);
        setTimeout(() => setShowGpsMessage(false), 7000);
    ...
    fetch("http://localhost:3001/api/connect-gps")
      ...
      .then((data) => {
        if (!data.success)
          throw new Error(
            data.message || "Fallo al iniciar GPS desde servidor."
          );
      })
      .catch((error) => {
        clearConnectionAttemptTimeout();
        setGpsMessageText(
          error.message || "Error de red/servidor al conectar GPS."
        );
        setShowGpsMessage(true);
        setIsAttemptingConnection(false);
        setTimeout(() => setShowGpsMessage(false), 7000);
  ...
  const getGpsButtonIcon = () => {
    if (isAttemptingConnection)
      return <AiOutlineLoading3Quarters className="rotating-icon" />;
    return <MdOutlineDeveloperBoard />;
  };

  const menuItems = [
    ...(currentUser
      ? [{ name: "Mi Cuenta", path: "/account", icon: <FaUserCircle /> }]
      : [
          {
            name: "Cuenta",
            icon: <FaUserCircle />,
    ...
    {
      name: arePredefinedRoutesVisible
        ? "Ocultar Rutas del Mapa"
        : "Mostrar Rutas en Mapa",
      icon: arePredefinedRoutesVisible ? <FaTrashAlt /> : <FaRoute />,
      action: onTogglePredefinedRoutes, // 🔥 Conexión con GoogleMaps
    },
  ];
  ...
  const renderMenuItem = (item, index) => {
    ...
    const linkClass = `link ${item.submenu && isExpanded ? "expanded" : ""} ${
      item.className || ""
    }`;
    const isDisabled =
      item.name === getGpsButtonText() &&
      isAttemptingConnection &&
      !gpsConnected;
            ...
            {isExpanded &&
              item.submenu.map((subitem, subindex) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    isActive ? "link sublink active" : "link sublink"
                  }
                >
  ...
  return (
    <>
      <div
        ...
        style={{ width: isOpen ? "var(--sidebar-width)" : "60px" }}
      >
        <div className="top">
